...
- 各印刷ピースは、2D プロファイルの段階で 1 つの連結領域になっていなければならない。特にダブテール等の継手ソケットをクリアランス分 `offset()` で拡大して切り抜くと、斜め辺が外形線に達した箇所が先細りになり、一点接続の破片（スライスすると分離・脱落する）ができやすい
- 継手ソケットと外形線の間には 1mm 以上の肉を残すか、外形まで完全に切り抜く。ノズル幅（0.4mm）未満の細い残り身も印刷されない
- エクスポート前に、各ピース STL の連結成分数が 1（プレート STL ならピース数と一致）であることを確認する。スライサーで目視するか、STL 頂点の union-find で機械的にチェックできる
